...
bool  HelloWorldScene::init()
{
	...
	meriam->setPosition(Vec2(384, 220));
	...
	listener_keyboard->onKeyReleased = CC_CALLBACK_2(HelloWorldScene::onKeyReleased, this);
	...
	listener_touch->onTouchBegan = CC_CALLBACK_2(HelloWorldScene::onTouchBegan, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(listener_touch, this);

	this->schedule(schedule_selector(HelloWorldScene::AddMeteor), 1);
	this->scheduleUpdate();
...
bool HelloWorldScene::onTouchBegan(Touch * touch, Event * event)
{
	if (nyawa > 0) {
		...
		auto projectile = Sprite::create("peluru10.png");
...
void HelloWorldScene::AddMeteor(float dt)
{
	...
	komet->runAction(Sequence::create(MoveTo::create(speed, Vec2(posX, 190)), CallFunc::create(CC_CALLBACK_0(HelloWorldScene::MeteorPass, this, komet)), nullptr));
...
void HelloWorldScene::MeteorPass(Sprite * komet)
{
	//buat efek ledakan
	//buat sprite animasi meledak
	auto sprite = Sprite::create("meledak1.png");
